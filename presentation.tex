...
\title[Автопостроение расписания]{Автоматическое построение расписания на minikanren}

% То, что в квадратных скобках, отображается в левом нижнем углу. 
\institute[СПбГУ]{}

% То, что в квадратных скобках, отображается в левом нижнем углу.
\author[Габдрахманов Азат]{Габдрахманов Азат Райнурович, 241 группа}
...
\begin{frame}
  \includegraphics[width=1.4cm]{SPbGU_Logo.png}
\vspace{-35pt}
\hspace{-10pt}
\begin{center}
   \begin{tabular}{c}
...
\begin{frame}[fragile]  
  \frametitle{Введение}
  \begin{itemize}
    \item  нужно строить расписание занятий
    %Краткий обзор тематики работы (как вариант --- устно, пока показывается титульный слайд)
    \item есть множество требований и ограничений, которые нужно учитывать при его построении
    \item Идея- строить расписание автоматически по начальным данным
...
\begin{frame}
  ...
  \textbf{Целью} работы является создание программы?, строящей расписание (и штуки,чтобы записывало в таблицу) %озвученной выше  

  \textbf{Задачи}:
  \begin{itemize}
    \item Разработать алгоритм, ищущий расписание %звучит очень круто, но мб несколько некорректно, я ведь не разрабатывал алогитм, мне его просто сказали
    \item построить расписание и вывести в таблицу %Карташова еще говорила, что она была бы очень сильна рада, если бы в таблицке была следующая функций:Например, она ставит предмет преподу, но у него уже занята эта пара, тогда бы программа мигала как-нибудь, показывая, что пара занята
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize} %нужно ли пытаться вставить фоточку
...
\multirow{-2}*{\!\includegraphics[width=6.8cm]{out3.pdf}}
\\
\includegraphics[width=2.5cm]{minikanren.png}
&
\\      
Грамматика: &
\\
...
\begin{frame}[fragile]
  ...
  \end{itemize}
  \begin{center}
  \includegraphics[width=0.8\textwidth]{Activ_SEL_Processing.pdf}
  \end{center}
\end{frame}
